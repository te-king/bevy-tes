...
/// Parser combinator: read `n` bytes and wrap them (NUL-trimmed) as an [`L1Str`].
/// Used for the format's fixed-length `char[n]` fields (e.g. object/spell names).
pub fn fixed_l1str<'a>(n: usize) -> impl Fn(&'a [u8]) -> IResult<&'a [u8], &'a L1Str> {
    move |input| {
...
///
/// The lifetime is threaded through so `T` may borrow from `data` (zero-copy output).
...
/// Wrap a subrecord field's bytes as an [`L1Str`], stopping at the first NUL byte.
///
...
/// size. No decoding happens here — the Windows-1252 bytes are borrowed as-is and
/// transcoded lazily via [`L1Str::decode`].
pub fn l1(bytes: &[u8]) -> &L1Str {
    let end = bytes.iter().position(|&b| b == 0).unwrap_or(bytes.len());
    L1Str::from_bytes(&bytes[..end])
}
