...
/// The bundled game data lives under `beth-rs/tests` (relative to the workspace root,
/// which is also this crate's manifest dir).
const ASSET_ROOT: &str = "beth-rs/tests";
...
fn loads_morrowind_esm_through_asset_server() {
    ...
    assert!(matches!(state, LoadState::Loaded), "unexpected load state: {state:?}");
